                        ...
                        @foreach($shoes as $shoe)
                            <div class="col-lg-4 col-sm-6">
                                <div class="product-item">
                                    <a href="{{ route('shop.show', $shoe->slug) }}">
                                        <div class="pi-pic">
                                            <div class="tag-sale">ON SALE</div>
                                            <img src="{{ asset('images/shoes/'.$shoe->slug.'.jpg') }}" alt="" class="product-image">
                                            <div class="pi-links">
                                                <form action="{{ route('WishlistController.store') }}" method="POST" class="form-in-line">
                                                    {{ csrf_field() }}
                                                    <input type="hidden" name="id" id="id" value="{{ $shoe->id }}">
                                                    <input type="hidden" name="name" id="name" value="{{ $shoe->name }}">
                                                    <input type="hidden" name="price" id="price" value="{{ $shoe->price }}">
